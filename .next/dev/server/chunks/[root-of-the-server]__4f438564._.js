...
"[project]/src/app/api/v1/smf/probes/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
// SMF Probe Data API
// =============================================================================
// GET /api/v1/smf/probes
// Returns probe attempt data and statistics
__turbopack_context__.s([
    "GET",
    ()=>GET
]);
...
async function GET() {
    try {
        ...
        if (isPostgres) {
            // Get total probe attempts
            const totalResult = await db.pool.query(`SELECT COUNT(*) as count FROM route_attempts WHERE is_probe = true`);
            const totalProbes = parseInt(totalResult.rows[0].count);
            // Get recent probe attempts (last 20)
            const recentResult = await db.pool.query(`SELECT 
          id, 
          facilitator_id, 
          phase, 
          result, 
        ...
        LIMIT 20`);
            // Get probe attempts by facilitator
            const byFacilitatorResult = await db.pool.query(`SELECT 
          facilitator_id,
          COUNT(*) as total,
          SUM(CASE WHEN result = 'success' THEN 1 ELSE 0 END) as successes,
          SUM(CASE WHEN result != 'success' THEN 1 ELSE 0 END) as failures,
          AVG(latency_ms)::integer as avg_latency_ms,
          MIN(created_at) as first_probe,
          MAX(created_at) as last_probe
        FROM route_attempts 
        WHERE is_probe = true 
        GROUP BY facilitator_id`);
            // Get probe attempts by result
            const byResultResult = await db.pool.query(`SELECT 
          result,
          COUNT(*) as count
        FROM route_attempts 
        WHERE is_probe = true 
        GROUP BY result
        ORDER BY count DESC`);
            // Get probe attempts in last hour
            const lastHourResult = await db.pool.query(`SELECT COUNT(*) as count 
        FROM route_attempts 
        WHERE is_probe = true 
        AND created_at > NOW() - INTERVAL '1 hour'`);
            const probesLastHour = parseInt(lastHourResult.rows[0].count);
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                summary: {
                    total: totalProbes,
                    lastHour: probesLastHour
                },
                byFacilitator: byFacilitatorResult.rows.map((row)=>({
                        facilitatorId: row.facilitator_id,
                        total: parseInt(row.total),
                        successes: parseInt(row.successes),
                        failures: parseInt(row.failures),
                        successRate: row.total > 0 ? (row.successes / row.total * 100).toFixed(1) + '%' : '0%',
                        avgLatencyMs: row.avg_latency_ms,
                        firstProbe: row.first_probe,
                        lastProbe: row.last_probe
                    })),
                byResult: byResultResult.rows.map((row)=>({
                        result: row.result,
            ...
            // SQLite
            const totalStmt = db.db.prepare(`SELECT COUNT(*) as count FROM route_attempts WHERE is_probe = 1`);
            const totalProbes = totalStmt.get().count;
            const recentStmt = db.db.prepare(`SELECT 
          id, 
          facilitator_id, 
          phase, 
          result, 
        ...
        ORDER BY created_at DESC 
        LIMIT 20`);
            const recentProbes = recentStmt.all();
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                summary: {
                    total: totalProbes,
                    lastHour: 'N/A (SQLite)'
                },
                recentAttempts: recentProbes.map((row)=>({
                        id: row.id,
                        facilitatorId: row.facilitator_id,
                        phase: row.phase,
                        result: row.result,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__4f438564._.js.map