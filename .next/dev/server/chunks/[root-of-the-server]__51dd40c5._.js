...
"[project]/src/lib/endpoint-validation.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";

// =============================================================================
// Endpoint Validation Utilities
// =============================================================================
// Validation and limits for API endpoints
__turbopack_context__.s([
    "checkWebhookLimit",
    ()=>checkWebhookLimit,
    "validateDateRange",
...
const MAX_WEBHOOKS_PER_API_KEY = 50;
...
function validateDateRange(startDate, endDate) {
    ...
    if (daysDiff > MAX_LOOKBACK_DAYS) {
        errors.push(`Date range cannot exceed ${MAX_LOOKBACK_DAYS} days`);
    }
    // Prevent queries too far in the past
    const maxPastDate = new Date();
    maxPastDate.setDate(maxPastDate.getDate() - MAX_LOOKBACK_DAYS);
    if (start < maxPastDate) {
        errors.push(`startDate cannot be more than ${MAX_LOOKBACK_DAYS} days in the past`);
    }
    // Prevent future dates
    const now = new Date();
    if (end > now) {
        errors.push('endDate cannot be in the future');
    }
    return {
...
function validatePagination(limit, offset) {
    ...
    if (limit !== null) {
        ...
        if (limit > MAX_PAGINATION_LIMIT) {
            errors.push(`limit cannot exceed ${MAX_PAGINATION_LIMIT}`);
        }
    }
    if (offset !== null) {
        if (offset < 0) {
...
function validateWebhookUrl(url) {
    ...
    if (url.length > MAX_WEBHOOK_URL_LENGTH) {
        errors.push(`URL cannot exceed ${MAX_WEBHOOK_URL_LENGTH} characters`);
    }
    try {
        const parsed = new URL(url);
        // Only allow HTTPS in production (can be configured)
...
function validateWebhookEvents(events) {
    ...
    if (events.length > 20) {
        errors.push('Cannot subscribe to more than 20 event types per webhook');
    }
    const validEvents = [
        'payment.verified',
        'payment.failed',
    ...
    for (const event of events){
        if (typeof event !== 'string') {
            errors.push('All events must be strings');
            break;
        }
        if (!validEvents.includes(event)) {
            errors.push(`Invalid event type: ${event}. Valid types: ${validEvents.join(', ')}`);
        }
    }
    return {
        valid: errors.length === 0,
...
function checkWebhookLimit(currentCount) {
    if (currentCount >= MAX_WEBHOOKS_PER_API_KEY) {
        return {
            valid: false,
            errors: [
                `Maximum of ${MAX_WEBHOOKS_PER_API_KEY} webhooks per API key`
            ]
        };
    }
    return {
...
function validateGranularity(granularity) {
    ...
    if (granularity && ![
        ...
        errors.push('granularity must be one of: hour, day, week');
    }
    return {
        valid: errors.length === 0,
        errors
...
function validatePeriodType(periodType) {
    ...
    if (periodType && ![
        ...
        errors.push('periodType must be one of: daily, weekly, monthly, custom');
    }
    return {
        valid: errors.length === 0,
        errors
...
"[project]/src/app/api/v1/metered/[endpointId]/analytics/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";

return __turbopack_context__.a(async (__turbopack_handle_async_dependencies__, __turbopack_async_result__) => { try {

// =============================================================================
// Metered Endpoint Analytics API
// =============================================================================
// GET /api/v1/metered/[endpointId]/analytics
// 
// Provides comprehensive analytics for a metered endpoint including:
// - Usage statistics (requests, payments, revenue)
// - Time-series data (hourly/daily breakdowns)
// - Error rates and status code breakdowns
// - Performance metrics (latency percentiles)
// - Payment method breakdown
// - Facilitator usage stats
__turbopack_context__.s([
    "GET",
    ()=>GET
]);
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$endpoint$2d$validation$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/endpoint-validation.ts [app-route] (ecmascript)");
var __turbopack_async_dependencies__ = __turbopack_handle_async_dependencies__([
    __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$auth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__,
...
[__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$auth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$request$2d$logging$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$db$2f$metered$2d$endpoints$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$integrations$2f$x402$2f$x402$2d$call$2d$log$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__] = __turbopack_async_dependencies__.then ? (await __turbopack_async_dependencies__)() : __turbopack_async_dependencies__;
;
;
;
;
...
async function GET(request, { params }) {
    const ctx = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$request$2d$logging$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["extractRequestContext"])(request, `/api/v1/metered/${params.endpointId}/analytics`);
    try {
        ...
        // Check if user has access to this endpoint (admin or endpoint owner)
        // For now, allow any authenticated user - can add ownership checks later
        if (apiKey.role !== 'admin') {
        // TODO: Add endpoint ownership check
        ...
        // Validate date range
        const dateValidation = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$endpoint$2d$validation$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateDateRange"])(startDateParam, endDateParam);
        ...
        const granularityValidation = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$endpoint$2d$validation$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateGranularity"])(granularityParam);
        if (!granularityValidation.valid) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'Invalid granularity',
                code: 'INVALID_GRANULARITY',
                details: granularityValidation.errors
            }, {
                status: 400
            });
        }
        const startDate = startDateParam || new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
        const endDate = endDateParam || new Date().toISOString();
        const granularity = granularityParam;
        // Get basic usage stats
        const usageStats = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$db$2f$metered$2d$endpoints$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getUsageStats"])(endpointId, startDate, endDate);
        // Get recent payments
        const recentPayments = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$db$2f$metered$2d$endpoints$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getPaymentsForEndpoint"])(endpointId, 100);
        // Get x402 call logs for detailed analytics
        const x402Logs = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$integrations$2f$x402$2f$x402$2d$call$2d$log$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getX402CallLogs"])(endpointId, {
            startDate,
            endDate,
            limit: 1000
        });
        // Calculate detailed metrics
        const metrics = calculateMetrics(x402Logs, recentPayments, usageStats, granularity);
        // Build response
        const response = {
            ...
            summary: {
                ...
                totalPayments: usageStats.totalPayments,
                totalRevenue: usageStats.totalRevenue,
                averageResponseTime: usageStats.averageResponseTime,
                successRate: calculateSuccessRate(x402Logs),
                errorRate: calculateErrorRate(x402Logs)
            },
            timeSeries: metrics.timeSeries,
            breakdowns: {
                byStatus: metrics.byStatus,
                byFacilitator: metrics.byFacilitator,
            ...
            performance: {
                latency: {
                    ...
                    average: usageStats.averageResponseTime
                }
            },
            ...includeDetails && {
                ...
                recentLogs: x402Logs.slice(0, 20).map((log)=>({
                        id: log.id,
...
/**
 * Calculate detailed metrics from logs and payments
 */ function calculateMetrics(logs, payments, usageStats, granularity) {
    // Time series data
    const timeSeries = {};
    // Breakdowns
    const byStatus = {};
    ...
    const byErrorCode = {};
    // Latency data for percentiles
    const latencies = [];
    // Process logs
    for (const log of logs){
        // Time series
        const timeKey = getTimeKey(log.timestamp, granularity);
        ...
        timeSeries[timeKey].requests++;
        if (log.status === 'success') {
            timeSeries[timeKey].payments++;
            const amount = BigInt(log.amount || '0');
            const currentRevenue = BigInt(timeSeries[timeKey].revenue);
            timeSeries[timeKey].revenue = (currentRevenue + amount).toString();
        }
        if (log.status === 'failed') {
            timeSeries[timeKey].errors++;
        }
        // Latency
        if (log.latencyMs !== undefined && log.latencyMs !== null) {
            latencies.push(log.latencyMs);
            const currentAvg = timeSeries[timeKey].averageLatency;
            const count = timeSeries[timeKey].requests;
            timeSeries[timeKey].averageLatency = (currentAvg * (count - 1) + log.latencyMs) / count;
        }
        // Breakdowns
    ...
    // Process payments for payment method breakdown
    // Note: We can't distinguish session vs payment from logs alone
    // This would need to be tracked in usage_logs
    byPaymentMethod['payment'] = payments.length;
 ...
 */ function getTimeKey(timestamp, granularity) {
    ...
    switch(granularity){
        ...
        case 'week':
            ...
            return weekStart.toISOString().slice(0, 10) + 'T00:00:00Z';
        default:
            return date.toISOString().slice(0, 10) + 'T00:00:00Z';
    }
}
...
//# sourceMappingURL=%5Broot-of-the-server%5D__51dd40c5._.js.map