...
async function GET() {
    try {
        ...
        try {
            ...
            const isPostgres = 'pool' in db || typeof db.pool?.query === 'function';
            if (isPostgres) {
                const result = await db.pool.query('SELECT id, name, status FROM facilitators WHERE status = $1', [
                    'active'
                ]);
                facilitators = result.rows;
            } else {
                const stmt = db.db.prepare('SELECT id, name, status FROM facilitators WHERE status = ?');
                facilitators = stmt.all('active');
            }
        } catch (error) {
            logger.warn({
                error
        ...
        const healthStatus = await Promise.all(facilitators.map(async (facilitator)=>{
            try {
                ...
                // Determine overall status based on snapshot
                let status = 'healthy';
                let reasons = [];
                if (snapshot) {
                    ...
                    // No snapshot data - assume healthy but mark as unknown
                    status = 'healthy';
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d3122aa8._.js.map