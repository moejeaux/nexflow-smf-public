...
"[project]/src/app/api/v1/smf/probes/detailed/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";

return __turbopack_context__.a(async (__turbopack_handle_async_dependencies__, __turbopack_async_result__) => { try {

// =============================================================================
// SMF Detailed Probe Data API
// =============================================================================
// GET /api/v1/smf/probes/detailed
// Returns all probe attempts with full details, no pagination limits
...
const logger = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$logger$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createLogger"])({
    component: 'SMFProbesDetailedAPI'
});
...
async function GET(request) {
    try {
        ...
        const limit = parseInt(searchParams.get('limit') || '1000', 10); // Default 1000, max reasonable
        ...
        if (isPostgres) {
            // Get all detailed probe attempts with route information
            let query = `
        SELECT 
          ra.id,
          ra.route_id,
          ...
          r.client_id,
          r.agent_id,
          r.selected_facilitator_id,
          r.created_at as route_created_at,
          r.completed_at as route_completed_at
        FROM route_attempts ra
        LEFT JOIN routes r ON ra.route_id = r.id
        WHERE ra.is_probe = true
      `;
            const queryParams = [];
            let paramIndex = 1;
            // Add time filter
            if (hours > 0) {
                query += ` AND ra.created_at > NOW() - INTERVAL '${hours} hours'`;
            }
            // Add filters
            if (facilitatorId) {
                query += ` AND ra.facilitator_id = $${paramIndex++}`;
                queryParams.push(facilitatorId);
            }
            if (network) {
                query += ` AND r.network = $${paramIndex++}`;
                queryParams.push(network);
            }
            if (token) {
                query += ` AND r.token = $${paramIndex++}`;
                queryParams.push(token);
            }
            if (resultFilter) {
                query += ` AND ra.result = $${paramIndex++}`;
                queryParams.push(resultFilter);
            }
            query += ` ORDER BY ra.created_at DESC LIMIT $${paramIndex++}`;
            queryParams.push(Math.min(limit, 10000)); // Cap at 10k for safety
      ...
      `;
            ...
            if (hours > 0) {
                countQuery += ` AND ra.created_at > NOW() - INTERVAL '${hours} hours'`;
            }
            if (facilitatorId) {
                countQuery += ` AND ra.facilitator_id = $${countParamIndex++}`;
                countParams.push(facilitatorId);
            }
            if (network) {
                countQuery += ` AND r.network = $${countParamIndex++}`;
                countParams.push(network);
            }
            if (token) {
                countQuery += ` AND r.token = $${countParamIndex++}`;
                countParams.push(token);
            }
            if (resultFilter) {
                countQuery += ` AND ra.result = $${countParamIndex++}`;
            ...
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                ...
                probes: result.rows.map((row)=>({
                        ...
                        route: {
                            ...
                            agentId: row.agent_id,
                            selectedFacilitatorId: row.selected_facilitator_id,
                            createdAt: row.route_created_at,
                            completedAt: row.route_completed_at
                        }
                    })),
            ...
            // SQLite fallback
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'SQLite not supported for detailed probe data',
                message: 'Use PostgreSQL for full probe data access'
            }, {
                status: 400
            });
        }
        ...
        logger.error({
            error
        }, 'Failed to get detailed probe data');
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0e5eb77d._.js.map